# ===========================================================
# 命名・リージョン・リソース配置に関する変数定義例
# ===========================================================

# リージョン略号（名前生成用）
region_code    = "jpe"

# リソース名につける識別番号（ゼロ埋め推奨）
sequence       = "001"

# ===========================================================
# Azure配置リージョン（リソース作成先）
# ===========================================================
region = "japaneast"

# ===========================================================
# Subscription情報（Step0: 既存または新規作成制御）
# ===========================================================
# 新規作成する場合のみ true、既存流用なら false
# EAではサブスクリプショ作成のロジックが大きく異なるためサブスクリプション作成のみ別ロジック
create_subscription   = true
#create_subscription   = false

# SpokeサブスクリプションID（既存利用時に指定）
#spoke_subscription_id = "ebc7f56b-e245-4d10-b49a-59293a1e67f5"

# Workload種別
subscription_workload = "Production"

# 課金情報
#EA環境では異なる想定
billing_account_name  = "0ae846b2-3157-5400-bf84-d255f8f82239:d68ce096-f337-4c84-9d39-05562a37bab0_2019-05-31"
billing_profile_name  = "IAMZ-4Q5A-BG7-PGB"
invoice_section_name  = "6HB2-O3GL-PJA-PGB"

# ===========================================================
# VNet / Subnet / NSG構成
# ===========================================================
# IPAM構成情報（VNet/Subnet共用）
ipam_subscription_id         = "7d62589c-474e-4dd8-8602-8e989750650f"
ipam_rg_name                 = "rg-is-hubnw-prd-jpe-001"
ipam_network_manager_name    = "vnm-is-hubnw-prd-jpe-001"
ipam_pool_name               = "ipam-is-terraform-jpe-001"

# VNet/サブネット/BastionサブネットのIP数
vnet_number_of_ips            = 1024   # /22
public_subnet_number_of_ips   = 64     # /26
private_subnet_number_of_ips  = 256    # /24
bastion_subnet_number_of_ips  = 64     # /26

# VPNクライアント許可CIDR、RDP許可ポート
vpn_client_pool_cidr = "172.16.201.0/24"
allowed_port         = 3389

# ===========================================================
# Hub Peering構成情報
# ===========================================================
hub_subscription_id = "7d62589c-474e-4dd8-8602-8e989750650f"
hub_rg_name         = "rg-is-hubnw-prd-dev-001"
hub_vnet_name       = "vnet-is-hubnw-prd-dev-001"
