...
variable "vnet_type" {
  description = "VNet のタイプ（public / private）"
  type        = string
}
...
# ===========================================================
locals {
  name_route_table         = var.project_slug != "" ? "rt-${var.project_slug}-${lower(var.vnet_type)}-${var.environment_id}-${var.region_code}-${var.sequence}" : null
