...
# -----------------------------------------------------------
# IPAM設定（分割した個別変数）
variable "ipam_subscription_id" {
  description = "IPAMが配置されているサブスクリプションID"
  type        = string
}
variable "ipam_rg_name" {
  description = "IPAMが配置されているリソースグループ名"
  type        = string
}
variable "ipam_network_manager_name" {
  description = "IPAMネットワークマネージャー名"
  type        = string
}
variable "ipam_pool_name" {
  description = "IPAMプール名"
  type        = string
}
...
# Subnet + NSG（Stage3）
# -----------------------------------------------------------
// 環境別のサブネットサイズ（常時こちらを使用）
variable "public_subnet_number_of_ips" {
  description = "Public VNet の通常サブネット IP 数（例: 64≒/26）"
  type        = number
  default     = 64
}

variable "private_subnet_number_of_ips" {
  description = "Private VNet の通常サブネット IP 数（例: 256≒/24）"
  type        = number
  default     = 256
}
