...
locals {
  ...
  # 分割されたIPAM変数から完全なipam_pool_idを構築
...
resource "azapi_resource" "subscription" {
  ...
  body = jsonencode({
    properties = {
      displayName  = var.subscription_display_name != "" ? var.subscription_display_name : "subs-${local.base}"
...
module "vnet-peering" {
  ...
  # VNet、ネットワーキング、ルーティングが全て完了後に実行
  depends_on = [module.vnet, module.networking, module.route-table]
}
